...
function getPackagedAppBundlePath() {
  ...
  if (!bundle.endsWith(".app") || !fs.existsSync(bundle)) return null;
  return bundle;
}

/** After a successful Open, drop quarantine so later launches do not need Terminal. */
function stripAppQuarantine() {
  const bundle = getPackagedAppBundlePath();
  if (!bundle) return;

  execFile("xattr", ["-p", "com.apple.quarantine", bundle], { timeout: 4000 }, (probeErr) => {
    if (probeErr) return;
    execFile("xattr", ["-cr", bundle], { timeout: 8000 }, (err) => {
      if (err) {
        console.error("stripAppQuarantine", err);
      }
    });
  });
}
