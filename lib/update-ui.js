...
function maybeNotifyUpdateAvailable(version, notify) {
  ...
  showNotification({
    title: tr.t("getSettings().updateNotifyTitle"),
    body: tr.t("getSettings().updateNotifyBody", { version }),
    silent: false,
  });
}
...
function buildUpdateDialogDetail(tr) {
  const parts = [
    tr.t("getSettings().updateDialogCurrent", { version: getAppVersion() }),
    tr.t("getSettings().updateDialogLatest", { version: updateInfo.version }),
  ];
  if (updateInfo.downloadName) {
    parts.push(tr.t("getSettings().updateAsset", { name: updateInfo.downloadName }));
  }
  if (updateInfo.source === "auto" && isAutoUpdaterEnabled()) {
    if (updateInfo.status === "downloaded") {
      parts.push(tr.t("getSettings().updateHintReady"));
    } else {
      parts.push(tr.t("getSettings().updateHintAuto"));
    }
  } else {
    parts.push(tr.t("getSettings().updateHintManual"));
  }
...
function buildUpdateDialogPayload() {
  ...
  if (hasUpdate) {
    ...
    if (isReady) {
      actions.push({
        id: "install",
        label: tr.t("getSettings().updateInstall"),
        primary: true,
      });
    } else if (isAuto) {
      actions.push({
        id: "download",
        label: tr.t("getSettings().updateDownloadInApp"),
        primary: true,
      });
      actions.push({ id: "releases", label: tr.t("getSettings().updateOpenReleases") });
    } else {
      actions.push({
        id: "download",
        label: tr.t("getSettings().updateDownload"),
        primary: true,
      });
      actions.push({ id: "releases", label: tr.t("getSettings().updateViewRelease") });
    }
    actions.push({ id: "later", label: tr.t("getSettings().updateLater") });

    return {
      kind: isReady ? "ready" : "available",
      title: isReady
        ? tr.t("getSettings().updateReadyTitle", { version: updateInfo.version })
        : tr.t("getSettings().updateAvailableTitle", { version: updateInfo.version }),
      detail: isReady ? tr.t("getSettings().updateReadyDetail") : buildUpdateDialogDetail(tr),
      actions,
    };
  }
  ...
  if (updateLastError) {
    return {
      kind: "error",
      title: tr.t("getSettings().updateErrorTitle"),
      detail: updateLastError,
      actions: [
        { id: "retry", label: tr.t("getSettings().updateRetry"), primary: true },
        { id: "ok", label: tr.t("getSettings().updateDialogOk") },
      ],
    };
  }
  ...
  return {
    kind: "up_to_date",
    title: tr.t("getSettings().updateUpToDateTitle"),
    detail: tr.t("getSettings().updateUpToDateDetail", { version: getAppVersion() }),
    actions: [{ id: "ok", label: tr.t("getSettings().updateDialogOk"), primary: true }],
  };
}
...
async function openUpdatePromptWindow(payload) {
  ...
  updatePromptWindow = new BrowserWindow({
    ...
    title: payload.title || tr.t("getSettings().updateDialogTitle"),
    icon: getAppIconPath(),
...
async function runNativeUpdateDialog(payload) {
  ...
  const { response } = await showMessageBox(
    ...
    withAppDialogIcon({
      type: dialogType,
      title: tr.t("getSettings().updateDialogTitle"),
...
async function checkForUpdates({
  ...
  if (updateCheckInFlight) {
    if (showDialog) {
      ...
      const payload = {
        kind: "checking",
        title: tr.t("getSettings().updateCheckingTitle"),
        detail: tr.t("getSettings().updateCheckingDetail"),
        actions: [{ id: "ok", label: tr.t("getSettings().updateDialogOk"), primary: true }],
      };
      if (useInAppUpdateDialog()) {
...
async function promptInstallReadyUpdate() {
  ...
  showNotification({
    title: tr.t("getSettings().updateReadyNotifyTitle"),
    body: tr.t("getSettings().updateReadyNotifyBody", { version: updateInfo.version }),
    silent: false,
  });

  const payload = {
    kind: "ready",
    title: tr.t("getSettings().updateReadyTitle", { version: updateInfo.version }),
    detail: tr.t("getSettings().updateReadyDetail"),
    actions: [
      { id: "install", label: tr.t("getSettings().updateInstall"), primary: true },
      { id: "later", label: tr.t("getSettings().updateLater") },
    ],
  };
